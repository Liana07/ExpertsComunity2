 ...
 * for which we want to check min required length e.g array('username' => 4)
 * @return array, containing all errors
 */
...
function show_errors($form_errors_array){
    $errors = "<p><ul style='padding-left:45px; padding-top: 0' class='alert alert-danger' '>";
...
function uploadAvatar($username){
    ...
    if(getimagesize($_FILES['avatar']['tmp_name'])){
        ...
        $filename = $username.hash('sha256',microtime()).".{$ext}";

        $path = $uploaddir.$filename; //demo.jpg
...
function _token(){
    $randonToken1 = bin2hex(openssl_random_pseudo_bytes(32));
    $randonToken=hash('sha256', $randonToken1);
