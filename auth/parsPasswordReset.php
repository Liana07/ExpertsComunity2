...
if(isset($_POST['passwordResetBtn'], $_POST['token'])){
    ...
    if(validate_token($_POST['token'])){
        ...
        } elseif (isset($_POST['passwordRecoveryBtn'], $_POST['token'])) {
            if (validate_token($_POST['token'])) {
                ...
                if (empty($form_errors)) {
                    ...
                    try {
                        ...
                        if ($rs = $statement->fetch()) {
                            ...
                            $reset_token1 = strtoupper(preg_replace('/[^A-Za-z0-9\-]/', '', $random_string));
                            $reset_token= hash('sha256', $reset_token1);
                        ...
                        <body style="background-color:#CCCCCC; color:#000; font-family: Arial, Helvetica, sans-serif;
                                            line-height:1.8em;">
                        <h2>User Authentication: Exparts Community</h2>
                        ...
                        <p><a href="http://localhost/ExpertsComunity/auth/forgot_password.php"> Reset Password</a></p>
                        <p><strong>&copy;' . date('Y') . ' WebDev</strong></p>
                        </body>
                        </html>';
                            $mail->addAddress($email, $username);
                            $mail->Subject = "Password Recovery Message from WebDev";
                            $mail->Body = $mail_body;
